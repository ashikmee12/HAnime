...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"User {user_id} started the bot with args: {context.args}")
...
async def handle_web_app_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"Received web app data from user {user_id}: {data}")
    ...
    try:
        ...
        if ad_data.get('status') == 'ad_completed':
            ...
            logger.info(f"Ad completed for user {user_id}, file: {file_code}")
        ...
        elif ad_data.get('status') == 'ad_error':
            logger.error(f"Ad error for user {user_id}, file: {ad_data.get('file_code')}")
            await update.message.reply_text("❌ There was an error with the ad. Please try again.")
...
def main():
    ...
    logger.info("🤖 Bot is running with Flask on port 10000...")
    print("🤖 Bot is running with Flask on port 10000...")
    app.run_polling()
